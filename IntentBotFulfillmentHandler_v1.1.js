export async function handler(event) {
    ...
    try {
        ...
        // Intent Journey
        let intentJourneyHistory = genesysOutputContext?.parameters?.intentJourney || "";
		let intentJourney = intentJourneyHistory.split(',').map(item => item.trim()).filter(item => item);
		
        if (detectedIntent != previousIntent) {
            intentJourney.push(detectedIntent);
        }
